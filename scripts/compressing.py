...
def compress_chip_folders(
    chip_path, compress_type="Brightfield", output_zip_path=None
):
    """
    ...
    if not os.path.isdir(chip_path):
        raise ValueError(
            f"The specified chip path does not exist or is not a directory: {chip_path}"
        )

    # Default output zip path
    if output_zip_path is None:
        output_zip_path = os.path.join(
            os.path.dirname(chip_path), f"{os.path.basename(chip_path)}.zip"
        )
    ...
    with zipfile.ZipFile(output_zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
        for metasurface_path in tqdm(
            metasurface_folders, desc="Processing metasurface folders"
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Compress metasurface folders into a structured zip file."
    )
    parser.add_argument(
        "chip_path",
        type=str,
        help="Path to the chip folder containing metasurface folders.",
    )
    parser.add_argument(
        "compress_type",
        type=str,
        choices=["Brightfield", "Laser"],
        help="Folder type to compress (Brightfield or Laser).",
    )
    parser.add_argument(
        "--output_zip_path",
        type=str,
        default=None,
        help="Path to save the output zip file. Defaults to chip_X.zip in the same directory.",
    )

    args = parser.parse_args()

    output_zip_path = compress_chip_folders(
        args.chip_path, args.compress_type, args.output_zip_path
    )
