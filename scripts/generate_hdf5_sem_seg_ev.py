        ...
        try:
            ...
            for _, row in particles_positions_df.iterrows():
                ...
                try:
                    ...
                    axes_x = max(
                        1, int((row["xMax"] - row["xMin"]) / 2)
                    )  # Ensure non-zero
                    axes_y = max(
                        1, int((row["yMax"] - row["yMin"]) / 2)
                    )  # Ensure non-zero
                except KeyError:
                    ...
                    try:
                        ...
                        print(
                            f"Warning: CSV format not recognized. Columns available: {particles_positions_df.columns.tolist()}"
                        )
                        continue
...
def tif_to_hdf5(
    ...
    if datatype == "Brightfield":
        all_tif_files = sorted(
            glob.glob(os.path.join(data_path, "*_Brightfield.tif"))
        )
    elif datatype == "Laser":
        all_tif_files = sorted(
            glob.glob(os.path.join(data_path, "*_Laser.tif"))
        )
    else:
        raise ValueError(
            "Invalid datatype. Choose either 'Brightfield' or 'Laser'."
        )
    ...
    if not tif_files:
        raise FileNotFoundError(
            f"No numbered TIF files (e.g., 01.tif) found in {data_path}"
        )
    ...
    with h5py.File(output_hdf5_path, "w") as hdf5_file:
        ...
        for tif_file in tif_files:
            file_basename = os.path.basename(tif_file).split(".")[
                0
            ]  # e.g., "01" from "01.tif"
            ...
            mask_file = os.path.join(
                data_path, f"{file_basename}_FITC_mask.npy"
            )
            # Load TIF image
            try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate HDF5 files for TIF image and CSV mask patches."
    )
    parser.add_argument(
        "--datatype",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="2025_03_04",
        help="Folder Path for the input TIF files.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    output_hdf5_path = os.path.join(
        args.output_path, f"EV_{args.datatype.lower()}.hdf5"
    )
