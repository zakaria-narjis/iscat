                        ...
                        for mask_name in mask_names:
                            ...
                            if os.path.exists(csv_path):
                                if "2024_11_29" in Path(root).parts:
                                    print(
                                        "Found 2024_11_29, remapping classes Cy5 -> 3, FITC -> 0, TRITC -> 1"
                                    )
                                    csv_paths.append(
                                        (
                                            csv_path,
                                            class_idx_2024_11_29[mask_name],
                                        )
                                    )
                                else:
                                    csv_paths.append(
                                        (csv_path, class_idx[mask_name])
                                    )
                            else:
...
def radial_average(image, center, axes, target_size=None):
    ...
    ellipse_mask = (
        (x - center[0]) ** 2 / axes[0] ** 2
        + (y - center[1]) ** 2 / axes[1] ** 2
    ) <= 1
...
def process_dataset(
    ...
    with h5py.File(output_path, "w") as hf:
                ...
                for csv_path, class_idx in csv_paths:
                    ...
                    for _, row in df.iterrows():
                        ...
                        if use_radial:
                            ...
                            processed = average_region(
                                region, averaging_axis, target_size
                            )
        ...
        if data_list:
            ...
            # Save metadata
            hf.attrs["averaging_method"] = (
                "radial" if use_radial else f"axis_{averaging_axis}"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process particle data and create HDF5 dataset"
    )
    parser.add_argument(
        "--datatype",
        type=str,
    ...
    output_hdf5_path = os.path.join(
        args.output_path, f"{args.datatype.lower()}_particles_{avg_type}.hdf5"
    )
    # Process dataset
    process_dataset(
        all_file_pairs,
