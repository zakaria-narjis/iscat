...
def extract_patch_instances(bbox_data, patch_coords, class_id):
    ...
    patch_bboxes["xMax"] = (patch_bboxes["xMax"] - x_min).clip(
        upper=x_max - x_min
    )
    patch_bboxes["yMax"] = (patch_bboxes["yMax"] - y_min).clip(
        upper=y_max - y_min
    )
...
def nd2_to_hdf5(nd2_paths, output_hdf5_path, patch_size=(256, 256), overlap=0):
    ...
    with h5py.File(output_hdf5_path, "w") as hdf5_file:
        ...
        hdf5_file.attrs["description"] = (
            "Image and mask patches with instance segmentation annotations."
        )
        ...
        for nd2_path in nd2_paths:
            ...
            if is_special_case:
                ...
                bbox_paths = {
                    original: os.path.join(
                        nd2_dir,
                        rename_mapping[original].replace("_mask.npy", ".csv"),
                    )
                    for original in mask_order
                }
            else:
                ...
                bbox_paths = {
                    mask: os.path.join(
                        nd2_dir, mask.replace("_mask.npy", ".csv")
                    )
            ...
            bbox_data = {
                mask_name: load_bbox_data(nd2_dir, mask_name)
                for mask_name in mask_order
            }
            ...
            for y in range(
                ...
                for x in range(
                    ...
                    for class_id, mask_name in enumerate(mask_order):
                        ...
                        patch_bbox_data = extract_patch_instances(
                            bbox_data[mask_name], patch_coords, class_id
                        )
                    ...
                    if image_dataset is None:
                        ...
                        annotations_dataset = hdf5_file.create_dataset(
                            ...
                            dtype=h5py.special_dtype(
                                vlen=str
                            ),  # Variable length string for JSON
                        )
                    ...
                    if patch_instances:
                        ...
                        annotations_dataset.resize(
                            annotations_dataset.shape[0] + 1, axis=0
                        )
    ...
    print(
        f"Image, mask patches, and instance annotations saved to {output_hdf5_path}"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate HDF5 files for image and mask patches with instance segmentation."
    )
    parser.add_argument(
        "--datatype",
        type=str,
    ...
    output_hdf5_path = os.path.join(
        args.output_path, f"{args.datatype.lower()}_instance.hdf5"
    )
    nd2_to_hdf5(
        nd2_paths,
        output_hdf5_path,
