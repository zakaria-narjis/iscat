...
from src.trainers.trainerseg import Trainer
...
def ddp_main(rank, world_size, args, config):
    ...
    elif config["model"]["type"] == "CellViT":
        model = CellViT(
            num_classes=num_classes,
            patch_size=config["model"]["cellvit"]["patch_size"],
            embed_dim=config["model"]["cellvit"]["embed_dim"],
            input_channels=in_channels,
            depth=config["model"]["cellvit"]["depth"],
            num_heads=config["model"]["cellvit"]["num_heads"],
            extract_layers=config["model"]["cellvit"]["extract_layers"],
        ).to(device)
    else:
    ...
    # Only rank 0 performs visualization and saves results
    if rank == 0:
