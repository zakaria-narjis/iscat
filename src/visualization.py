...
def plot_image_with_masks(
    image, predicted_mask, ground_truth_mask, output_path="Cy5.png"
):
    """
    ...
    for class_label, color in predicted_colors.items():
        ...
        combined_image = overlay_mask(
            combined_image, class_mask, color=color, alpha=0.5
        )
    ...
    for class_label, color in predicted_colors.items():
        legend_elements.append(
            plt.Line2D(
                ...
                lw=2,
                label=f"Pred Class {class_label}",
            )
        )
    for class_label, color in gt_colors.items():
        legend_elements.append(
...
def batch_plot_images_with_masks(
    images, predicted_masks, ground_truth_masks, output_dir="output"
):
    for idx, (image, predicted_mask, ground_truth_mask) in enumerate(
        zip(images, predicted_masks, ground_truth_masks)
    ):
        output_path = f"{output_dir}/image_{idx}.png"
        plot_image_with_masks(
            image[0], predicted_mask, ground_truth_mask, output_path
        )
...
def predict(model, dataset, device="cpu", images_indicies=[0, 1, 2, 4]):
    ...
    for idx in images_indicies:
        while True:
            image, mask = dataset[
                idx
            ]  # (image: torch.Size([3, 256, 256]), mask: torch.Size([3, 256, 256]))
            if 1 in mask:
                break
        input_image = image.to(device).unsqueeze(
            0
        ...
        with torch.no_grad():
            ...
            if output.shape[1] == 1:
                ...
                predicted_mask = (
                    predicted_mask.squeeze(0).squeeze(0).cpu().numpy()
                )  # Shape: (256, 256)
            else:
                predicted_mask = torch.argmax(
                    output.squeeze(0), dim=0
                )  # Shape: (256, 256)
                predicted_mask = (
