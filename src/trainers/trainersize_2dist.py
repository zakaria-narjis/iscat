...
from src.losses import knnDivergence, MonotonicityLoss, mmd_loss
import logging
import os
...
def generate_label_distribution(
    ...
    """
    points_80 = torch.zeros(10000)  # Initialize a tensor to store valid points
    points_300 = torch.zeros(10000)  # Initialize a tensor to store valid points
...
class TrainerSize(nn.Module):
    ...
    def train_epoch(
        ...
        for batch_images, particles_cls in train_dataloader:
            ...
            # knn_loss_80 = mmd_loss(
            #     batch_predictions[particles_cls==0],
            #     target_distribution[:10000],
            # )
            # knn_loss_300 = mmd_loss(
            ...
            2, len(batch_predictions[particles_cls==0])//4, dtype=torch.int
        ),
                method="absolute",
            )
            knn_loss_300 = knnDivergence(
                ...
                target_distribution[10000:],
                k_neighbors=torch.arange(
            2, len(batch_predictions[particles_cls==1])//4, dtype=torch.int),
                method="absolute",
            )
            knn_loss= knn_loss_80 + 2*knn_loss_300
            # Compute monotonicity loss
            # moseparation_loss = 0notonicity_loss = MonotonicityLoss(
            #     batch_predictions, batch_images, increasing=True
            # )
            monotonicity_loss = MonotonicityLoss(
                batch_predictions[particles_cls==1], batch_images[particles_cls==1], increasing=True
            )+ MonotonicityLoss(
                batch_predictions[particles_cls==0], batch_images[particles_cls==0], increasing=True
            )
        ...
        epoch_logs = {
            "avg_loss": avg_loss,
            "avg_knn_loss": avg_knn_loss,
            "avg_monotonicity_loss": avg_monotonicity_loss,
        }
        return epoch_logs
    ...
    def train(self, train_dataloader: DataLoader, num_epochs: int):
        ...
        for epoch in tqdm(
            ...
            loss_log.append(epoch_logs["avg_loss"])
            current_lr = self.optimizer.param_groups[0]["lr"]
            # Learning rate scheduling
            self.scheduler.step(epoch_logs["avg_loss"])

            # Log the loss
            self.logger.info(
                f"Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_logs['avg_loss']:.4f}, LR: {current_lr:.2e}"
            )
            self.logger.info(
                f"Epoch [{epoch+1}/{num_epochs}], KNN Loss: {epoch_logs['avg_knn_loss']:.4f}, Monotonicity Loss: {epoch_logs['avg_monotonicity_loss']:.4f}"
            )
            if self.writer:
                self.writer.add_scalar('Loss/train', epoch_logs['avg_loss'], epoch)

            # Early stopping check
            if epoch_logs["avg_loss"] < best_loss:
                best_loss = epoch_logs["avg_loss"]
                torch.save(
                    {
                        "epoch": epoch,
