...
class Trainer:
    def __init__(
        ...
        self.fine_tuning_enabled = config.get("fine_tuning", {}).get(
            "enabled", False
        )
        self.freeze_encoder = config.get("fine_tuning", {}).get(
            "freeze_encoder", False
        )
        self.encoder_lr_factor = config.get("fine_tuning", {}).get(
            "encoder_lr_factor", 0.1
        )
        self.progressive_unfreeze = (
            config.get("fine_tuning", {})
            .get("progressive_unfreeze", {})
            .get("enabled", False)
        )
        self.epochs_per_layer = (
            config.get("fine_tuning", {})
            .get("progressive_unfreeze", {})
            .get("epochs_per_layer", 5)
        )
    ...
    def _setup_fine_tuning(self):
        ...
        if hasattr(self.model, "Conv1"):
            self.encoder_modules = [
                ...
                (
                    ("Conv5", self.model.Conv5)
                    if hasattr(self.model, "Conv5")
                    else None
                ),
            ]
            self.encoder_modules = [
                m for m in self.encoder_modules if m is not None
            ]

        # Log the identified encoder modules
        self.logger.info(
            f"Identified encoder modules: {[name for name, _ in self.encoder_modules]}"
        )
    ...
    def _freeze_encoder(self, up_to_layer=None):
        ...
        layers_to_freeze = (
            ...
            else self.encoder_modules[: up_to_layer + 1]
        )
    ...
    def _initialize_optimizer(self):
        ...
        decoder_params = [
            p for p in self.model.parameters() if p not in encoder_params_set
        ]
        ...
        self.logger.info(
            f"Initialized optimizer with encoder LR={base_lr * self.encoder_lr_factor}, decoder LR={base_lr}"
        )
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        if (
            ...
            if layer_to_unfreeze < len(self.encoder_modules):
                ...
                self.logger.info(
                    f"Epoch {epoch}: Progressive unfreezing up to layer {layer_to_unfreeze}"
                )
    ...
    def train(self, train_loader, val_loader, num_epochs):
        ...
        if self.fine_tuning_enabled:
            self.logger.info(
                f"Fine-tuning enabled: Encoder freeze={self.freeze_encoder}, "
                f"Encoder LR factor={self.encoder_lr_factor}, "
                f"Progressive unfreeze={self.progressive_unfreeze}"
            )
            if self.progressive_unfreeze:
                self.logger.info(
                    f"Progressive unfreezing: {self.epochs_per_layer} epochs per layer"
                )
        ...
        for epoch in tqdm(
            ...
            if val_loss < best_val_loss:
                ...
                no_improve = 0
                self.logger.info(
                    f"Saved best model at epoch {epoch+1} with val_loss={val_loss:.4f}"
                )
            else:
                no_improve += 1
            ...
            if (
                ...
                self.logger.info(
                    f"LR Encoder: {self.optimizer.param_groups[0]['lr']:.2e}, "
