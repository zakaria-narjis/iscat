...
class Trainer:
    def __init__(
        ...
        # Use AdamW optimizer like in CellViT trainer
        self.optimizer = optim.AdamW(
            model.parameters(), 
            lr=config["optimizer"]["parameters"]["lr"],
            weight_decay=config["optimizer"]["parameters"].get("weight_decay", 0.01),
            betas=config["optimizer"]["parameters"].get("betas", (0.9, 0.999))
        )
        
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer,
            mode="min",
            factor=config["scheduler"]["parameters"]["factor"],
            patience=config["scheduler"]["parameters"]["patience"],
        )
    ...
    def train(self, train_loader, val_loader, num_epochs):
        ...
        for epoch in tqdm(
            ...
            # Distributed reduction - gather metrics from all processes
            tensors = [
            ...
            if self.is_main_process:
                ...
                # Merge class metrics from all processes
