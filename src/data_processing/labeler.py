...
class Labeler:
    ...
    def __init__(self, method="comdet"):
        if method == "comdet":
            ...
            if os.path.exists(fiji_path):
                self.ij = imagej.init(
                    ij_dir_or_version_or_endpoint=fiji_path,
                    mode=imagej.Mode.HEADLESS,
                )  # for Linux
            else:
                raise FileNotFoundError(
                    f"Fiji.app not found in the specified path: {fiji_path}"
                )
    ...
    def generate_labels(
        self, fluo_images_paths, seg_args=None, segmentation_method="comdet"
    ):
        """
        ...
        if segmentation_method == "comdet":
            ...
            for fluo_image_path, args in zip(fluo_images_paths, seg_args):
                ...
                if not os.path.exists(particles_position_path):
                    ...
                    particles_positions_df = pd.read_csv(
                        particles_position_path
                    )
                else:
                    particles_positions_df = pd.read_csv(
                        particles_position_path
                    )
                mask = self.create_labels_mask(
                    canvas_shape, particles_positions_df
                )
            ...
            for fluo_image_path in fluo_images_paths:
                ...
                if image is None:
                    raise FileNotFoundError(
                        f"Image not found at {fluo_image_path}"
                    )
                ...
                kmeans_result = kmeans_color_quantization(
                    image_normalized, clusters=2
                )

                # Apply Otsu's thresholding to get binary mask
                kmeans_normalized = cv2.normalize(
                    kmeans_result,
                    None,
                    0,
                    255,
