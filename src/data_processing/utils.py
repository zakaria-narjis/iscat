...
class Utils:
    ...
    def get_data_paths(
        ...
        image_indices: Union[List[int], int] = [0, 100, 200],
    ):
        """
        ...
        for dirpath, dirnames, filenames in os.walk(root_path):
            if os.path.basename(dirpath) == mode:
                if type(image_indices) is list:
                    zimages_files.append(
                        [
                            os.path.join(dirpath, f"frame_{idx}.tiff")
                    ...
                    zimages_files.append(
                        [
                            os.path.join(
                                dirpath,
                                f"frame_{idx}_{image_indices}_mean.tiff",
                            )
                            for idx in range(image_indices)
                        ]
                    )
                else:
                ...
                if not all(os.path.exists(path) for path in zimages_files[-1]):
                    ...
                    for file in filenames:
                        if file.endswith(".nd2"):
                            ...
                            Utils.process_nd2_file(
                                nd2_path, dirpath, image_indices
                            )
                            break
                    if nd2_path is None:
                ...
                assert all(
                    os.path.exists(path)
                    for path in [cy5_path, fitc_path, tritc_path]
                )
    ...
    def process_nd2_file(
        nd2_file: str, dirpath: str, indices: Union[List[int], int]
    ) -> None:
        """
        ...
        with nd2.ND2File(nd2_file) as f:
            ...
            if type(indices) is list:
                ...
                for idx in indices:
                    ...
                    output_path = os.path.join(dirpath, f"frame_{idx}.tiff")
                ...
                for idx, frame in enumerate(frames):
                    output_path = os.path.join(
                        dirpath, f"frame_{idx}_{indices}_mean.tiff"
                    )
                    imwrite(output_path, frame, dtype="uint16")
                    print(f"Saved: {output_path}")
            else:
    ...
    @staticmethod
    def generate_np_masks(
        all_fluo_images_paths, seg_args=None, seg_method="comdet"
    ):
        if seg_method == "comdet":
            labeler = Labeler(method="comdet")
            if seg_args is None:
                args = {"ch1i": True, "ch1a": 4, "ch1s": 10}
            else:
                args = seg_args
            seg_args = [[args, args, args]] * len(all_fluo_images_paths)
            for fluorescence_images_path, seg_arg in tqdm(
                zip(all_fluo_images_paths, seg_args),
                total=len(all_fluo_images_paths),
                desc="Creating Masks with ComDet",
            ):
                labeler.generate_labels(
                    fluorescence_images_path,
                    seg_arg,
                    segmentation_method=seg_method,
                )
        elif seg_method == "kmeans":
            labeler = Labeler(method="kmeans")
            for fluorescence_images_path in tqdm(
                all_fluo_images_paths,
                total=len(all_fluo_images_paths),
                desc="Creating Masks with KMeans",
            ):
                labeler.generate_labels(
                    fluorescence_images_path, segmentation_method=seg_method
                )
        else:
