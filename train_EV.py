...
def get_args_parser(add_help: bool = True):
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/ev_seg_config.yaml",
...
def create_dataloaders(
    train_dataset, valid_dataset, test_dataset, batch_size, config
):
...
def write_config_to_tensorboard(writer, config):
    ...
    important_params = {
        ...
        "Training": [
            ...
            (
                "training.fine_tuning.encoder_lr_factor",
                "Encoder Learning Rate Factor",
            ),
            (
                "training.fine_tuning.progressive_unfreeze.enabled",
                "Progressive Unfreezing",
            ),
            (
                "training.fine_tuning.progressive_unfreeze.epochs_per_layer",
                "Epochs Per Layer Unfreeze",
            ),
        ],
        "Model": [
            ...
            if isinstance(param, tuple):
                ...
                try:
                    ...
                    if value is not None:
                        table_rows.append(
                            f"|{display_name}|{format_value(value)}|"
                        )
...
def main(args):
    ...
    if config["data"]["data_type"] == "Brightfield":
        hdf5_path = os.path.join(
            config["data"]["dataset_folder_path"], "EV_brightfield.hdf5"
        )
    elif config["data"]["data_type"] == "Laser":
        hdf5_path = os.path.join(
            config["data"]["dataset_folder_path"], "EV_Laser.hdf5"
        )
    ...
    if config["model"]["type"] == "U_Net":
        ...
        model = R2U_Net(img_ch=in_channels, output_ch=out_channels)
    else:
    ...
    fine_tuning_config = {
        ...
        "progressive_unfreeze": {"enabled": False, "epochs_per_layer": 0},
    }
    ...
    if config.get("training", {}).get("fine_tuning", {}).get("enabled", False):
        ...
        fine_tuning_config["freeze_encoder"] = config["training"][
            "fine_tuning"
        ].get("freeze_encoder", False)
        fine_tuning_config["encoder_lr_factor"] = config["training"][
            "fine_tuning"
        ].get("encoder_lr_factor", 0.1)
        fine_tuning_config["progressive_unfreeze"]["enabled"] = (
            config["training"]["fine_tuning"]
            .get("progressive_unfreeze", {})
            .get("enabled", False)
        )
        fine_tuning_config["progressive_unfreeze"]["epochs_per_layer"] = (
            config["training"]["fine_tuning"]
            .get("progressive_unfreeze", {})
