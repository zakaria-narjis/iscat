...
class BalancedBatchSampler(Sampler):
    def __init__(self, labels, batch_size, num_classes):
        assert batch_size % 2 == 0, "Batch size must be divisible by number of classes"
        ...
        # Shuffle each class's indices initially
        for indices in self.class_indices.values():
            random.shuffle(indices)

        self.class_cursors = {k: 0 for k in self.class_indices}

        # Total batches to yield (minimum class length determines this)
        self.num_batches = min(len(indices) for indices in self.class_indices.values()) // self.samples_per_class
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            ...
            for cls in range(self.num_classes):
                ...
                next_cursor = cursor + self.samples_per_class

                # Reshuffle if needed
                if next_cursor > len(indices):
                    random.shuffle(indices)
                    cursor = 0
                    next_cursor = self.samples_per_class

                batch.extend(indices[cursor:next_cursor])
                self.class_cursors[cls] = next_cursor
...
def create_dataloaders(train_dataset, batch_size, num_workers=0, use_balanced_sampler=True):
    if use_balanced_sampler:
        # Extract labels from dataset
        labels = [train_dataset[i][1] for i in range(len(train_dataset))]
        sampler = BalancedBatchSampler(labels, batch_size=batch_size, num_classes=2)
        train_loader = DataLoader(
            train_dataset,
